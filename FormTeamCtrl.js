app.controller('FormTeamController', function ($scope) {

    var _this = this;
    _this.team = [];
    _this.selectedDomain = null;
    _this.selectedGender = null;
    _this.selectedAvailability = null;
    _this.filteredUsers = [];

    _this.currentPage = 1;
    _this.usersPerPage = 20;
    _this.users = [
        ...
        { "id": 13, "first_name": "Candice", "last_name": "Harbar", "email": "charbarc@home.pl", "gender": "Female", "avatar": "https://robohash.org/autarchitectotenetur.png?size=50x50&set=set1", "domain": "Sales", "available": false },
        ...
        { "id": 40, "first_name": "Anet", "last_name": "Dearing", "email": "pdearing13@cam.ac.uk", "gender": "Male", "avatar": "https://robohash.org/laudantiumdoloreos.png?size=50x50&set=set1", "domain": "UI Designing", "available": false },
    ...
    _this.getUniqueDomains = function () {
        var uniqueDomains = [];
        angular.forEach(_this.users, function (user) {
            if (uniqueDomains.indexOf(user.domain) === -1) {
                uniqueDomains.push(user.domain);
            }
        });
        return uniqueDomains;
    };
    _this.filterUsers = function () {
        _this.filteredUsers = _this.users.filter(function (user) {
            var domainMatch = !_this.selectedDomain || user.domain === _this.selectedDomain;
            var genderMatch = !_this.selectedGender || user.gender === _this.selectedGender;
            var availabilityMatch = _this.selectedAvailability === null || user.available === _this.selectedAvailability;

            var nameMatch = !_this.searchName || (user.first_name + ' ' + user.last_name).toLowerCase().includes(_this.searchName.toLowerCase());
            return domainMatch && genderMatch && availabilityMatch && nameMatch;
        });
        _this.currentPage = 1;
        _this.totalPages = Math.ceil(_this.filteredUsers.length / _this.usersPerPage);
        _this.usersPerPage = _this.filteredUsers.length <= 20 ? _this.filteredUsers.length : 20;

    };
    _this.filterUsers();

    _this.addToTeam = function (user) {
        var isDomainUnique = _this.team.every(function (teamMember) {
        ...
        if (isDomainUnique) {
            ...
            Swal.fire({
                position: "top-end",
                icon: "success",
                title: user.first_name + " " + "added to team",
                showConfirmButton: false,
                timer: 500
            });
            setTimeout(function() {
                _this.showTeamDetails();
            }, 1500);
        } else {
            Swal.fire({
                position: "top-end",
                icon: "error",
                title: user.domain + " " + "domain is already in team",
                showConfirmButton: false,
                timer: 1000
            });
        }
    ...
    _this.showTeamDetails = function () {
        ...
        //     });
        // var teamMembersString = teamMemberNames.join(',');
        var teamDetailsModal = new bootstrap.Modal(document.getElementById('teamDetails'));
